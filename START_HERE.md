...
The first command reports the environment, worktree, verification, and task state.
Run `./repo setup` when its diagnostics identify missing dependencies. Use
`./repo verify` for the complete local gate. See docs/repo-operations.md for
focused checks and publication. The second command identifies the next
